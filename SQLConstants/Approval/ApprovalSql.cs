...
public class ApprovalSql
{
    ...
    public const string GET_MY_APPROVALS = "Select PreviousCancelRemark,ReGenerated,IF(`Status`='Rejected',IF(App1Status='Rejected',App1Name,IF(App2Status='Rejected',App2Name,IF(App3Status='Rejected',App3Name,App4Name))),'') 'RejectBy',IF(`Status`='Rejected', IFNULL(RejectionReason,'N/A'),'') AS 'RejectReason' ,RelativePersonID,RelativePersonName,ReferenceNo,`Session`,CampusName,Purpose,TotalItem,DATE_FORMAT(ExtendedBillDate,'%d %b, %y') 'ExeOn',TotalAmount,`Status`,IniName,DATE_FORMAT(AppDate,'%d %b, %y') 'AppDate',DATE_FORMAT(IniOn,'%d %b, %y') 'IniOn',App1Name,App2Name,App3Name,App4Name,App1Status,App2Status,App3Status,App4Status,IFNULL(DATE_FORMAT(App1DoneOn,'%d %b, %y'),'NA') 'App1On',IFNULL(DATE_FORMAT(App2DoneOn,'%d %b, %y'),'NA') 'App2On',IFNULL(DATE_FORMAT(App3DoneOn,'%d %b, %y'),'NA') 'App3On',IFNULL(DATE_FORMAT(App4DoneOn,'%d %b, %y'),'NA') 'App4On',CancelledReason,DATE_FORMAT(CancelledOn,'%d %b, %y') 'CancelledOn',CancelledBy,CloseReason,DATE_FORMAT(CloseOn,'%d %b, %y') 'CloseOn',CloseBy,IF(`Status`='Pending' And HOUR(TIMEDIFF(now(),IniOn))>=48,'Y','N') 'FinalStat',BudgetRequired,BudgetAmount,PreviousTaken,CurStatus,BudgetStatus,BudgetReferenceNo,BillId,BillRequired,VendorID,ForDepartment,Note,MyType,ExtendedBillDate,AdditionalName,AppCat,CONCAT(IFNULL(CONCAT(App1Name,', '),''),IFNULL(CONCAT(App2Name,', '),''),IFNULL(CONCAT(App3Name,', '),''),IFNULL(CONCAT(App4Name,', '),'')) 'Auth',DATE_FORMAT(AppDate,'%m/%d/%Y') 'AD',DATE_FORMAT(ExtendedBillDate,'%m/%d/%Y') 'EBD',FirmName,FirmPerson,FirmContactNo,ReferenceBillStatus,R_Status as 'RecievingStatus',DATE_FORMAT(R_LastRcvOn,'%d.%m.%Y %r')'LastRecieveOn',CampusCode from purchaseapprovalsummary where true @Condition  ORDER BY IniOn,`Status`  LIMIT @LimitItems OFFSET @OffSetItems";
